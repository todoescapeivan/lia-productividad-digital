# Módulo 1: Herramientas Básicas

## Objetivo
Aprender a usar herramientas digitales esenciales para la comunicación y organización.

## Contenido
- Crear y usar una cuenta de correo electrónico.
- Usar WhatsApp Web para comunicarse desde la computadora.
- Guardar y compartir archivos en Google Drive.

## Ejercicio práctico
1. Crear una cuenta de Gmail.  
2. Enviar un correo a un compañero.  
3. Subir un archivo a Google Drive y compartirlo.  
# Proyecto Productividad Digital

## 🎯 Objetivo
Apoyar a adultos mayores y personas con experiencia para que sigan productivos, aprendan herramientas digitales, trabajen desde casa y moneticen su conocimiento.  
Creado por Juan Carlos Páez, con la guía de Lia (IA educativa).

## 📚 Módulos educativos
- **Módulo 1:** Herramientas básicas (correo, WhatsApp Web, Google Drive).  
- **Módulo 2:** Trabajo remoto (Zoom, Teams, Trello).  
- **Módulo 3:** Monetización digital (plataformas freelance, cursos online).  
- **Módulo 4:** Seguridad digital (contraseñas, estafas comunes, pagos seguros).  

## 📂 Estructura del repositorio
# Proyecto Productividad Digital

## 🎯 Objetivo
Apoyar a adultos mayores y personas con experiencia para que sigan productivos, aprendan herramientas digitales, trabajen desde casa y moneticen su conocimiento.  
Creado por Juan Carlos Páez, con la guía de Lia (IA educativa).

## 📚 Módulos educativos
- **Módulo 1:** Herramientas básicas (correo, WhatsApp Web, Google Drive).  
- **Módulo 2:** Trabajo remoto (Zoom, Teams, Trello).  
- **Módulo 3:** Monetización digital (plataformas freelance, cursos online).  
- **Módulo 4:** Seguridad digital (contraseñas, estafas comunes, pagos seguros).  

## 📂 Estructura del repositorio

# Módulo 1: Herramientas Básicas

## Objetivo
Aprender a usar herramientas digitales esenciales para la comunicación y organización.

## Contenido
- Crear y usar una cuenta de correo electrónico.
- Usar WhatsApp Web para comunicarse desde la computadora.
- Guardar y compartir archivos en Google Drive.

## Ejercicio práctico
1. Crear una cuenta de Gmail.  
2. Enviar un correo a un compañero.  
3. Subir un archivo a Google Drive y compartirlo.  

